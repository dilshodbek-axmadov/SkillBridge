...
import { Mail, Lock, Eye, EyeOff, User, Zap, TrendingUp, BarChart3, Target, Check, Code2, Briefcase, Loader2 } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const { register, loginWithGoogle, loading, error, clearError, isAuthenticated, user, fetchUser } =
    useAuthStore();
  ...
  const handleGoogle = async () => {
    ...
    setLocalError('');
    if (!googleConfigured) {
      setLocalError('Google sign-in is not configured. Please contact support.');
      return;
    }
    setGoogleLoading(true);
    try {
      const accessToken = await getGoogleAccessToken();
      const data = await loginWithGoogle(accessToken, form.user_type);
      if (data?.user) {
        navigate(getHomePathForUser(data.user), { replace: true });
      }
    } catch (e) {
      const fallback = e?.message || 'Could not sign in with Google.';
      // If the store already set an `error`, keep it; otherwise show fallback
