...
import { Mail, Lock, Eye, EyeOff, Zap, TrendingUp, BarChart3, Target, Loader2 } from 'lucide-react';
import useAuthStore from '../store/authStore';
import { getHomePathForUser } from '../utils/authPaths';
import { getGoogleAccessToken, isGoogleAuthConfigured } from '../utils/googleAuth';

export default function LoginPage() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const { login, loginWithGoogle, loading, error, clearError, isAuthenticated, user, fetchUser } =
    useAuthStore();
  ...
  const handleGoogle = async () => {
    ...
    clearError();
    if (!googleConfigured) {
      // Surface a clear error via the store so the UI shows it
    ...
    try {
      ...
      useAuthStore.setState({
        error: e?.message || 'Could not sign in with Google.',
      });
    } finally {
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-primary-600 via-primary-700 to-purple-600 relative overflow-hidden items-center justify-center p-12">
        ...
        <div className="relative z-10 max-w-md text-white">
          ...
          <p className="text-white/80 text-lg mb-10 leading-relaxed">
            Get personalized career guidance, skill gap analysis, and learning roadmaps powered by artificial intelligence.
          </p>
          ...
          <div className="space-y-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-semibold text-sm">Career Path Analysis</p>
                <p className="text-white/70 text-sm">AI-driven recommendations for your growth</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-white/15 backdrop-blur-sm rounded-lg flex items-center justify-center flex-shrink-0">
              ...
              <div>
                <p className="font-semibold text-sm">Real-Time Market Data</p>
                <p className="text-white/70 text-sm">Stay updated with industry trends</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-white/15 backdrop-blur-sm rounded-lg flex items-center justify-center flex-shrink-0">
              ...
              <div>
                <p className="font-semibold text-sm">Personalized Roadmaps</p>
                <p className="text-white/70 text-sm">Step-by-step learning plans tailored to you</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 bg-gray-50 dark:bg-gray-950">
        <div className="w-full max-w-md">
          {/* Logo (mobile only, desktop shows on left) */}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 p-8">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mb-8">Sign in to continue your career journey</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex justify-end">
                <Link
                  to="/forgot-password"
                  className="text-sm font-medium text-primary-600 hover:text-primary-700 no-underline transition-colors"
                >
                  Forgot password?
                </Link>
              </div>
            ...
            <button
              ...
              className="w-full h-[44px] bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 disabled:opacity-60 disabled:cursor-not-allowed transition-colors cursor-pointer flex items-center justify-center gap-3"
            >
              {googleLoading ? (
