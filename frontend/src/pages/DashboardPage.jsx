...
import {
  ...
  AlertCircle, Home, Map, Lightbulb, FileText, Settings,
  Menu, X, LogOut,
} from 'lucide-react';
      ...
      <nav className="flex-1 px-3 mt-2 space-y-1 overflow-y-auto">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`w-[18px] h-[18px] flex-shrink-0 ${active ? 'text-primary-600' : 'text-gray-400'}`} />
              {item.label}
            </Link>
          );
        })}
      ...
      <div className="flex-1 lg:ml-64">
        ...
        <header className="lg:hidden bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-20">
          <button
            ...
            className="p-1.5 text-gray-600 hover:text-gray-900 bg-transparent border-none cursor-pointer"
          >
        ...
        {/* Content */}
        <main className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 max-w-6xl">
