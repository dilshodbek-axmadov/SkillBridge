...
function formatDashboardSalary(min, max, currency = 'UZS') {
  const format = (value) => {
  ...
  if (minText && maxText) return `${minText}-${maxText} ${currency}`;
  if (minText) return `from ${minText} ${currency}`;
  if (maxText) return `up to ${maxText} ${currency}`;
  return '';
}
    ...
    <div className="bg-gradient-to-r from-primary-600 via-primary-700 to-purple-600 rounded-2xl p-6 sm:p-8 text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="text-sm">
          <p className="font-semibold">{t('dashboard.profileCompletion')}</p>
          {pct < 100 && (
            <Link to="/profile-setup" className="inline-flex items-center gap-1 text-white/80 hover:text-white text-xs no-underline mt-0.5">
              {t('dashboard.completeProfile')} <ChevronRight className="w-3 h-3" />
            </Link>
          )}
        </div>
      </div>
...
function QuickStats({ skills, hasGapAnalysis, primaryRoadmap, gapPendingLearning, recommendations }) {
  ...
  const cards = [
    ...
    },
    {
      ...
      sub: `${primarySkills} ${t('dashboard.primary')}`,
    ...
    },
    {
      ...
      label: t('dashboard.skillsToLearn'),
      sub: skillsToLearn > 0 ? t('dashboard.fromGapAnalysis') : (hasGapAnalysis ? t('dashboard.noOpenGaps') : t('dashboard.runAnalysisSeeGaps')),
    ...
    },
    {
      ...
      value: jobMatches,
      label: t('dashboard.jobMatches'),
      sub: t('dashboard.basedOnProfile'),
...
function YourSkills({ skills }) {
  ...
  return (
    <Section title={t('dashboard.yourSkills')} actionLabel={t('dashboard.viewAll')} actionTo="/manage-skills">
      ...
      <div className="flex gap-3 mt-3 text-xs text-gray-400 dark:text-gray-500">
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-500" /> {t('dashboard.expert')}</span>
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-blue-500" /> {t('dashboard.advanced')}</span>
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-500" /> {t('dashboard.intermediate')}</span>
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-gray-400" /> {t('dashboard.beginner')}</span>
      </div>
    </Section>
  );
}
...
function LearningRoadmap({ hasGapAnalysis, primaryRoadmap, topGapSkillNames }) {
  ...
  if (!hasGapAnalysis) {
    return (
      <Section title={t('dashboard.yourLearningRoadmap')}>
        <div className="text-center py-10 text-gray-400 dark:text-gray-500">
          <Target className="w-10 h-10 mx-auto mb-3 opacity-50" />
          <p className="text-sm mb-2 max-w-md mx-auto">
            {t('dashboard.runGapAnalysisDesc')}
          </p>
          <Link
            to="/skills-gap"
          ...
          >
            {t('dashboard.getRoadmap')} <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
      </Section>
    );
  ...
  if (!roadmapItems.length) {
    return (
      <Section title={t('dashboard.yourLearningRoadmap')} actionLabel={t('dashboard.viewGaps')} actionTo="/skills-gap">
        <div className="text-center py-8 text-gray-500 dark:text-gray-400">
          <p className="text-sm mb-4 max-w-lg mx-auto">
            {t('dashboard.skillGapsSaved')} {t('dashboard.generateStepByStep')}
          </p>
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg text-sm font-medium no-underline hover:bg-primary-700 transition-colors"
          >
            {t('dashboard.buildLearningRoadmap')} <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
      </Section>
    );
  ...
  const statusConfig = {
    completed: { dot: 'bg-emerald-500', badge: 'bg-emerald-100 text-emerald-700', label: t('dashboard.done') },
    learning: { dot: 'bg-emerald-500', badge: 'bg-emerald-100 text-emerald-700', label: t('dashboard.learning') },
    next: { dot: 'bg-orange-400', badge: 'bg-orange-100 text-orange-700', label: t('dashboard.next') },
    pending: { dot: 'bg-gray-300', badge: 'bg-gray-100 text-gray-500', label: t('dashboard.pending') },
  };

  return (
    <Section title={t('dashboard.yourLearningRoadmap')} actionLabel={t('dashboard.viewFullRoadmap')} actionTo="/roadmap">
      <div className="relative">
        ...
        <div className="space-y-6">
          {items.map((item) => {
            ...
            const pri = item.priority === 'high' ? t('dashboard.high') : item.priority === 'medium' ? t('dashboard.med') : null;
            return (
                      ...
                      <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400 mb-3">
                        {item.estHours > 0 && (
                          <span className="flex items-center gap-1">
                            ...
                            {t('dashboard.est')}. {item.estHours} h
                          </span>
                        )}
                      </div>
                      <Link
                      ...
                      <p className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500 dark:text-gray-400 mb-3">
                        {item.unlocksAfter && (
                          <span className="flex items-center gap-1">
                            ...
                            {t('dashboard.after')} {item.unlocksAfter}
                          </span>
                        )}
                        {item.estHours > 0 && (
                          <span className="flex items-center gap-1">
...
function SkillsGapAnalysis({ profile, recommendations, gaps }) {
  ...
  return (
    <Section
      title={`${t('dashboard.skillsToCloseFor')} ${desiredRole}`}
      subtitle={t('dashboard.fromLatestAnalysis')}
    >
      <div className="space-y-4">
        {preview.map((gap) => {
          const s = PRIORITY_STYLES[gap.priority] || PRIORITY_STYLES.medium;
          const importanceLabel = gap.importance === 'core' ? t('dashboard.core') : t('dashboard.secondary');
          const jobLine = gap.job_count > 0 ? `${gap.job_count} ${t('dashboard.postings30dTrend')}` : t('dashboard.marketDemandTracked');
          const reason = `${importanceLabel} ${t('dashboard.coreGap')} — ${jobLine}.`;
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500 dark:text-gray-400 mb-4">
                <span className="flex items-center gap-1">
                  ...
                  {t('dashboard.demandScore')} {typeof gap.demand_score === 'number' ? Math.round(gap.demand_score) : '—'}
                </span>
                {gap.growth_rate ? (
                  <span className="flex items-center gap-1">
                    <TrendingUp className="w-3 h-3" />
                    {t('dashboard.growth')} {gap.growth_rate}%
                  </span>
                ) : null}
              </div>
              <Link
              ...
              >
                {t('dashboard.viewRoadmapResources')}
              </Link>
            </div>
          );
        })}
      ...
      <Link to="/skills-gap" className="inline-flex items-center gap-1 text-primary-600 text-sm font-medium no-underline hover:underline mt-4">
        {t('dashboard.viewAllSkillGaps')} <ArrowRight className="w-4 h-4" />
      </Link>
    </Section>
  );
}
...
function RecentActivity({ activities }) {
  ...
  if (!activities?.length) {
    return (
      <Section title={t('dashboard.yourRecentActivity')} actionLabel={t('dashboard.viewAllActivity')} actionTo="/activity">
        <div className="text-center py-8 text-gray-400 dark:text-gray-500 text-sm">
          <p>{t('dashboard.noActivityYet')}</p>
          <p className="mt-1 text-xs">{t('dashboard.registerSetupProfile')}</p>
        </div>
      </Section>
    );
  }
...
function RecommendedJobs({ jobs }) {
  ...
  return (
    <Section title={`${t('dashboard.jobsMatchingProfile')} (${jobs.length})`} actionLabel={t('dashboard.viewAllJobs')} actionTo="/jobs">
      {jobs.length === 0 ? (
        <div className="text-center py-8 text-gray-400 dark:text-gray-500 text-sm">
          <p>{t('dashboard.noJobRecommendations')}</p>
          <p className="mt-1 text-xs">{t('dashboard.addPositionSkills')}</p>
        </div>
      ) : (
        <div className="space-y-3">
          {jobs.map((job) => {
            const salary = formatDashboardSalary(job.salary_min, job.salary_max, job.salary_currency);
            const matched = job.matched_skills || [];
            const missing = job.missing_skills || [];
            const totalSkills = (job.skills || []).length || (matched.length + missing.length);

            return (
              <div key={job.job_id} className="rounded-xl border border-gray-200 dark:border-gray-800 p-4 hover:shadow-md transition-shadow flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <div className="min-w-0">
                      <h3 className="text-sm font-bold text-gray-900 dark:text-gray-100 truncate">{job.job_title}</h3>
                      <p className="text-xs text-gray-500 dark:text-gray-400">{job.company_name || t('dashboard.company')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400 mt-2 flex-wrap">
                    {salary ? <span className="flex items-center gap-1"><TrendingUp className="w-3 h-3" />{salary}</span> : null}
                    {job.location ? <span>{job.location}</span> : null}
                    {totalSkills > 0 ? <span>{matched.length}/{totalSkills} skills</span> : null}
                  </div>
                  {missing.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {missing.slice(0, 2).map((skill) => (
                        <span key={skill} className="text-[10px] bg-red-50 text-red-600 px-1.5 py-0.5 rounded font-medium">{t('dashboard.missing')}: {skill}</span>
                      ))}
                      {missing.length > 2 && (
                        <span className="text-[10px] text-gray-400 dark:text-gray-500">+{missing.length - 2} {t('dashboard.more')}</span>
                      )}
                    </div>
                  )}
                </div>
                ...
                {job.match_percentage != null && (
                  <div className="relative w-10 h-10 flex-shrink-0">
                    <svg viewBox="0 0 36 36" className="w-10 h-10 -rotate-90">
                      ...
                      <circle cx="18" cy="18" r="15.9" fill="none" className={matchTrack(job.match_percentage)} strokeWidth="3.5" strokeDasharray={`${job.match_percentage} ${100 - job.match_percentage}`} strokeLinecap="round" />
                    </svg>
                    <span className={`absolute inset-0 flex items-center justify-center text-[10px] font-bold ${matchColor(job.match_percentage)}`}>{job.match_percentage}%</span>
                  </div>
                )}
              </div>
            );
...
function BottomCTA() {
  ...
  return (
    <div className="bg-gradient-to-r from-primary-50 via-purple-50 to-primary-50 dark:from-primary-900/20 dark:via-purple-900/20 dark:to-primary-900/20 rounded-2xl p-8 text-center">
      ...
      <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">{t('dashboard.readyToAccelerate')}</h2>
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const [summaryRes, statusRes, gapsRes, roadmapsRes, activityRes, jobsRes] = await Promise.all([
          ...
          api.get('/jobs/recommended/', { params: { limit: 3 } }).catch(() => ({ data: { jobs: [] } })),
        ]);
      ...
      <div className="space-y-6">
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <RecommendedJobs jobs={jobRecommendations} />
          </div>
        </div>
