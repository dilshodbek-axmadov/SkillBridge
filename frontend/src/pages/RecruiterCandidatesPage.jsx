...
export default function RecruiterCandidatesPage() {
  ...
  const getProfileCount = (count) => {
    return count === 1 ? t('recruiter.profile') : t('recruiter.profiles');
  };

  return (
    <RecruiterLayout user={user}>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t('recruiter.findCandidatesTitle')}</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          {t('recruiter.findCandidatesSubtitle')}
        </p>
      </div>
      ...
      <form onSubmit={handleSearch} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 sm:p-5 mb-6 space-y-4">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <input
            ...
            placeholder={t('recruiter.nameRoleKeyword')}
        ...
        {!loading && (
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {accessInfo?.truncated ? (
              <>
                {t('recruiter.showingOf', {
                  shown: total,
                  total: totalUnfiltered,
                  count: getProfileCount(totalUnfiltered)
                })}
              </>
            ) : (
              <>
                {t('recruiter.profilesMatch', { count: `${total} ${getProfileCount(total)}` })}
              </>
            )}
          </p>
        )}
      ...
      {accessInfo?.truncated && (
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-xl border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/30 px-4 py-3 text-sm text-amber-900 dark:text-amber-200">
          <div className="flex items-center gap-2">
            ...
            <span>
              {t('recruiter.freePlanLimit', {
                limit: accessInfo.developer_visibility_limit,
                total: totalUnfiltered
              })}
            </span>
          </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {c.is_saved ? (
                  ...
                  >
                    ...
                    {savingId === c.id ? t('recruiter.saving') : t('recruiter.save')}
                  </button>
                )}
                <Link
                  to={`/recruiter/candidates/${c.id}`}
